...
parser.add_argument('--trainpath', default="/home/nati/dataset/mvs_training/dtu",help='train datapath')
parser.add_argument('--testpath', default="/home/nati/dataset/mvs_training/dtu",help='test datapath')
...
parser.add_argument('--w', type=int, default=640, help='the number of source views')
parser.add_argument('--h', type=int, default=512, help='the number of source views')
...
parser.add_argument("--noRandom", default=False, action='store_true',
                        help='whether to pick random pixel when to generate ray')

parser.add_argument('--chunk_ratio', type=float, default=1)

parser.add_argument("--netchunk", type=int, default=10,
                        help='number of pts sent through network in parallel, decrease if running out of memory')

parser.add_argument("--n_rays", type=int, default=512*640,
                        help='number of pts sent through network in parallel, decrease if running out of memory')
...
parser.add_argument("--only_uniform_sampling", default=False, action='store_true',
                        help='whether to use only uniform sampling')
...
def test(model, model_loss, TestImgLoader, args):
    ...
    if (not is_distributed) or (dist.get_rank() == 0):
        ...
        if args.mode == 'test':
            save_scalars(logger, 'fulltest', avg_test_scalars.mean(), batch_idx)
...
def train_render_net(volume_feature,model_nerf,sample,optimizer,pseudo_depth,epoch_idx,args,loss_base,loss_aug):
    ...
    depth_loss = sl1loss(rays_depth, depth_pred, mask)
...
def train_begin(rank,args):
    ...
    model = CascadeMVSNet(refine=False, ndepths=[int(nd) for nd in args.ndepths.split(",") if nd],
                            ...
                            grad_method=args.grad_method, num_view = args.num_view)
    ...
    test_dataset = test_MVSDataset(args.testpath, args.testlist, "test", args.num_view+1, args.numdepth, args.interval_scale)
