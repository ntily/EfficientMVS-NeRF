...
class Rendering_Consistency_Net(nn.Module):
    ...
    def forward(self, volume_feature,pseudo_depth, batch):
        ...
        N_rays, N_samples = self.args.n_rays , self.args.N_samples   
        c2ws, w2cs, intrinsics = pose_ref['c2ws'], pose_ref['w2cs'], pose_ref['intrinsics']
        
        # ray_coordinate_world, ray_dir_world, colors, ray_coordinate_ref, depth_candidates, rays_os, rays_depths, ndc_parameters
        rays_pts, rays_dir, target_s, rays_NDC, depth_candidates, rays_o, rays_depth, ndc_parameters = \
                build_rays_norm(imgs, pseudo_depth_h, pose_ref, w2cs, c2ws, intrinsics, near_fars, N_rays, N_samples, pad=self.args.pad, 
                                isRandom = not self.args.noRandom, chunk_ratio = self.args.chunk_ratio, only_uniform_sampling= self.args.only_uniform_sampling, chunk=self.args.netchunk)
