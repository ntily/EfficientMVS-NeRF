...
# class CNNNeRF(nn.Module):
#   def __init__(self, norm_act=InPlaceABN, w=640, h=512, chunk_ratio=1, chunk=-1):
#       super(CNNNeRF, self).__init__()

#       self.conv = ConvBnReLU(86, 32, 3, 1, 1, norm_act=norm_act)
#       self.deconv = nn.ConvTranspose2d(32, 4, 3, padding=1)

#       if chunk_ratio>0:
#         self.h=int(h*chunk_ratio)
#         self.w=int(w*chunk_ratio)
#       else:
#         self.h=int(torch.sqrt(chunk))
#         self.w = self.h
        
    

#   def forward(self, x):
#       x = rearrange(x, '(h w) b c -> b c h w', h=self.h, w=self.w)

#       x = self.conv(x)
#       out = self.deconv(x)
#       out = rearrange(out, 'b c h w -> (h w) b c', h=self.h, w=self.w)
...
class CNNNeRF(nn.Module):
  def __init__(self, norm_act=InPlaceABN, w=640, h=512, chunk_ratio=1, chunk=-1):
      ...
      self.conv = ConvBnReLU(86, 64, 3, 1, 1, norm_act=norm_act)
      self.deconv1 = nn.ConvTranspose2d(64, 32, 3, padding=1)
      self.deconv2 = nn.ConvTranspose2d(32, 4, 3, padding=1)
  ...
  def forward(self, x):
      ...
      x = self.conv(x)
      out = self.deconv2(self.deconv1(x))
      out = rearrange(out, 'b c h w -> (h w) b c', h=self.h, w=self.w)
      out[..., :3] = torch.sigmoid(out[..., :3])
      out[..., 3] = torch.relu(out[..., 3])
...
class RenderNet(nn.Module):
    def __init__(self, D=8, W=256, input_ch_pts=3, input_ch_views=3, input_ch_feat=8, skips=[4], net_type='v2', w=640, h= 512, chunk_ratio=1, chunk=-1):
        """
        """
        ...
        if 'v0' == net_type:
            ...
            self.nerf = CNNNeRF(w=w, h=h, chunk_ratio=chunk_ratio, chunk=chunk)
...
def create_nerf_mvs(args, pts_embedder=True, use_mvs=False, dir_embedder=True,share_warp=False):
    ...
    model = RenderNet(D=args.netdepth, W=args.netwidth,
                 ...
                 net_type=args.net_type, w=args.w, h=args.h, chunk_ratio=args.chunk_ratio, chunk=args.netchunk).to(device)
