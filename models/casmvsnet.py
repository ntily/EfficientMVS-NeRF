...
class DepthNet(nn.Module):
    ...
    def forward(self, features, proj_matrices, depth_values, num_depth, cost_regularization,imgs,pad=0, prob_volume_init=None):
        ...
        volume_feat = F.interpolate(img_feat,size=[d,h,w],mode="trilinear",align_corners=True)
        # step 3. cost volume regularization
        volume_feat = cost_regularization(volume_feat)

        # cost_reg = F.upsample(cost_reg, [num_depth * 4, img_height, img_width], mode='trilinear')
        prob_volume_pre =self.prob(volume_feat).squeeze(1)
        ...
        return {"depth": depth, "photometric_confidence": photometric_confidence,"volume_feat":volume_feat}

class CascadeMVSNet(nn.Module):
    def __init__(self, refine=False, ndepths=[48, 32, 8], depth_interals_ratio=[4, 2, 1], share_cr=False,
                 grad_method="detach", arch_mode="fpn", cr_base_chs=[8, 8, 8], num_view = 3):
        ...
        if self.share_cr:
            self.cost_regularization = CostRegNet(in_channels=self.feature.out_channels + (num_view+1)*3, base_channels=8)
        else:
            self.cost_regularization = nn.ModuleList([CostRegNet(in_channels=self.feature.out_channels[i] + (num_view+1)*3,
    ...
    def forward(self, imgs, proj_matrices, depth_values):
        ...
        return outputs,outputs["stage1"]["volume_feat"]
...
class DepthNet_eval(nn.Module):
    ...
    def forward(self, features, proj_matrices, depth_values, num_depth, cost_regularization,imgs,pad=0, prob_volume_init=None):
        ...
        _,C,H,W = features[0].shape
        ref_feature, src_features = features[0], features[1:]
        # Only 4 images will be used in evaluation image features
        img_feat = torch.empty((B, 3*(num_views-1) + C, num_depth, *ref_feature.shape[-2:]), device=imgs.device, dtype=torch.float)
        # img_feat_no_ref = torch.empty((B, 3*(num_views-1) + C, num_depth, *ref_feature.shape[-2:]), device=imgs.device, dtype=torch.float)
        imgs = F.interpolate(imgs.view(B * num_views, *imgs.shape[2:]), (H, W), mode='bilinear', align_corners=False).view(B, num_views,-1,H,W).permute(1, 0, 2, 3, 4)
        img_feat[:, :3, :, pad:H + pad, pad:W + pad] = imgs[0].unsqueeze(2).expand(-1, -1, num_depth, -1, -1)
        # step 1. feature extraction
